...
window.drupalTranslations = {
    "strings": {
        "": {
            ...
            "Continue": "Continuar",
            "Done": "Hecho",
            "1 hour\u0003@count hours": "1 hora\u0003@count horas",
            "1 day\u0003@count days": "1 d\u00eda\u0003@count d\u00edas",
            "OK": "OK",
            "Prev": "Ant",
            "Mon": "Lun",
            "Tue": "Mar",
            ...
            "Hide": "Hide",
            "Unlink": "Desvincular",
            "1 year\u0003@count years": "1 a\u00f1o\u0003@count a\u00f1os",
            "1 week\u0003@count weeks": "1 semana\u0003@count semanas",
            "1 min\u0003@count min": "1 min\u0003@count mins",
            "1 sec\u0003@count sec": "1 seg\u0003@count segs",
            "Not promoted": "No publicado en p\u00e1gina principal",
            "mm\/dd\/yy": "mm\/dd\/yy",
            "Quick edit": "Edici\u00f3n r\u00e1pida",
            "1 month\u0003@count months": "1 mes\u0003@count meses",
            ...
            "Revert to default": "Volver a los valores predeterminados",
            ...
            "Zero items selected": "Cero elementos seleccionados",
            "All @count items selected": "Todos los @count elementos seleccionados",
            "Select all media": "Seleccionar todos los elementos multimedia",
            "Show media item weights": "Muestra los pesos de los elementos de contenido multimedia",
            "Hide media item weights": "Oculta los pesos de los elementos de contenido multimedia",
            "Downloads": "Descargas",
	...
	$(window).bind('scroll', function () {
 	...
 	$(document).ready(function() {
		...
		$('header .ico-menu').click(function() {
			...
			$('header .right .wrapper-menus').fadeToggle(100);
		});
		...
		$('.title-anchor').each(function(){
			if ($(this).prev().hasClass('anchor-point')) {
				$('.anchors .container').append('<div><a href="#'+$(this).prev().attr('id')+'">'+$(this).text()+'</a></div>');
			}
		})

		// Tabs

	  	$('<div class="wrapper-menu-tabs"><div class="ico-mobil-tabs"></div><div class="wrapper-menu"></div></div>').insertBefore('.wrapper-tabs');
	  	$('.wrapper-tabs .tab-content .title-tabs').clone().appendTo('.wrapper-menu-tabs .wrapper-menu');

	  	$('.wrapper-menu-tabs .wrapper-menu .title-tabs:first-child').each(function() {
	  		$(this).addClass('active');
	  	})

	  	$('.wrapper-menu-tabs .title-tabs').click(function() {
			var position = $(this).index();
			$('.wrapper-menu-tabs .title-tabs').removeClass('active');
			$(this).addClass('active');
			$('.tab-content').hide();
			$('.tab-content').eq(position).fadeIn();
	    });

	  	// Premis

	  	$('<div class="wrapper-premis-tabs"><div class="wrapper-menu"></div></div>').insertBefore('.modul-premios .wrappper-content');
	  	$('.modul-premios .wrappper-content .wrapper-year .year').clone().appendTo('.wrapper-premis-tabs .wrapper-menu');

	  	$('.wrapper-premis-tabs .year:first-child').each(function() {
	  		$(this).addClass('active');
	  	})

	  	$('.wrapper-premis-tabs .year').click(function() {
			var position = $(this).index();
			$('.wrapper-premis-tabs .year').removeClass('active');
			$(this).addClass('active');
			$('.wrapper-year').hide();
			$('.wrapper-year').eq(position).fadeIn();
	    });


		// Buscador personas

		let labelFiltroOficina = $('#views-exposed-form-buscador-equipo-entity-view-1 .form-item-oficina label').text();
		$('#views-exposed-form-buscador-equipo-entity-view-1 .form-item-oficina select option:first').text(labelFiltroOficina);
		$('#views-exposed-form-buscador-equipo-entity-view-1 .form-item-oficina label').text('');
...
;
(function($){$(document).ready(function(){var myLazyLoad=new LazyLoad({elements_selector:".lazy"})})})(jQuery);
...
(function(f) {
    f.fn.ddslick = function(m) {
        if (d[m]) {
            ...
            if (typeof m === "object" || !m) {
                ...
                f.error("Method " + m + " does not exists.")
            }
        }
    };
    var d = {},
    ...
    d.init = function(n) {
        var m = f.extend({}, e, n);
        if (f("#css-ddslick").length <= 0 && m.embedCSS) {
            f(c).appendTo("head")
        }
        return this.each(function() {
            var x = f.extend({}, e, n);
            ...
            if (!s) {
                ...
                r.find("option").each(function() {
                    ...
                    p.push({
                        text: f.trim(z.text()),
                        value: z.val(),
                        selected: z.is(":selected"),
                        description: y.description,
                        imageSrc: y.imagesrc
                    })
                });
                if (x.keepJSONItemsOnTop) {
                    f.merge(x.data, p)
                } else {
                    x.data = f.merge(p, x.data)
                }
                var o = r,
                    u = f("<div>").attr("id", r.attr("id") + "-dd-placeholder");
                r.replaceWith(u);
                r = u;
                r.addClass("dd-container").append(j).append(a);
                r.find("input.dd-selected-value").attr("id", f(o).attr("id")).attr("name", f(o).attr("name"));
                var p = r.find(".dd-select"),
                    w = r.find(".dd-options");
                w.css({
                    width: x.width
                });
                ...
                f.each(x.data, function(y, z) {
                    if (z.selected) {
                        x.defaultSelectedIndex = y
                    }
                    w.append('<li><a class="dd-option">' + (z.value ? ' <input class="dd-option-value" type="hidden" value="' + z.value + '" />' : "") + (z.imageSrc ? ' <img class="dd-option-image' + (x.imagePosition == "right" ? " dd-image-right" : "") + '" src="' + z.imageSrc + '" />' : "") + (z.text ? ' <label class="dd-option-text">' + z.text + "</label>" : "") + (z.description ? ' <small class="dd-option-description dd-desc">' + z.description + "</small>" : "") + "</a></li>")
                });
                var v = {
                    settings: x,
                    original: o,
                ...
                r.data("ddslick", v);
                if (x.selectText.length > 0 && x.defaultSelectedIndex == null) {
                    r.find(".dd-selected").html(x.selectText)
                } else {
                    var t = (x.defaultSelectedIndex != null && x.defaultSelectedIndex >= 0 && x.defaultSelectedIndex < x.data.length) ? x.defaultSelectedIndex : 0;
                    k(r, t)
                }
                r.find(".dd-select").on("click.ddslick", function() {
                    g(r)
                });
                r.find(".dd-option").on("click.ddslick", function() {
                    k(r, f(this).closest("li").index())
                });
                if (x.clickOffToClose) {
                    w.addClass("dd-click-off-close");
                    r.on("click.ddslick", function(y) {
                        y.stopPropagation()
                    });
                    f("body").on("click", function() {
                        f(".dd-open").removeClass("dd-open");
                        f(".dd-click-off-close").slideUp(50).siblings(".dd-select").find(".dd-pointer").removeClass("dd-pointer-up")
                    })
                }
            }
        })
    ...
    d.destroy = function() {
        return this.each(function() {
            ...
            if (n) {
                var m = n.original;
                o.removeData("ddslick").unbind(".ddslick").replaceWith(m)
            }
        })
    };

    function b(n, o) {
        var m = n.find(".dd-option-value[value= '" + o + "']").parents("li").prevAll().length;
        k(n, m)
    }

    function k(r, t) {
        var v = r.data("ddslick");
        var s = r.find(".dd-selected"),
            o = s.siblings(".dd-selected-value"),
            w = r.find(".dd-options"),
            m = s.siblings(".dd-pointer"),
            q = r.find(".dd-option").eq(t),
            n = q.closest("li"),
            p = v.settings,
            u = v.settings.data[t];
        r.find(".dd-option").removeClass("dd-option-selected");
        q.addClass("dd-option-selected");
        v.selectedIndex = t;
        v.selectedItem = n;
        v.selectedData = u;
        if (p.showSelectedHTML) {
            s.html((u.imageSrc ? '<img class="dd-selected-image' + (p.imagePosition == "right" ? " dd-image-right" : "") + '" src="' + u.imageSrc + '" />' : "") + (u.text ? '<label class="dd-selected-text">' + u.text + "</label>" : "") + (u.description ? '<small class="dd-selected-description dd-desc' + (p.truncateDescription ? " dd-selected-description-truncated" : "") + '" >' + u.description + "</small>" : ""))
        } else {
            s.html(u.text)
        }
        o.val(u.value);
        v.original.val(u.value);
        r.data("ddslick", v);
        l(r);
        h(r);
        if (typeof p.onSelected == "function") {
            p.onSelected.call(this, v)
        }
    }

    function g(q) {
        var p = q.find(".dd-select"),
            n = p.siblings(".dd-options"),
            m = p.find(".dd-pointer"),
            o = n.is(":visible");
        f(".dd-click-off-close").not(n).slideUp(50);
        f(".dd-pointer").removeClass("dd-pointer-up");
        p.removeClass("dd-open");
        if (o) {
            n.slideUp("fast");
            m.removeClass("dd-pointer-up");
            p.removeClass("dd-open")
        } else {
            p.addClass("dd-open");
            n.slideDown("fast");
            m.addClass("dd-pointer-up")
        }
        i(q)
    }

    function l(m) {
        m.find(".dd-select").removeClass("dd-open");
        m.find(".dd-options").slideUp(50);
        m.find(".dd-pointer").removeClass("dd-pointer-up").removeClass("dd-pointer-up")
    }

    function h(p) {
        var o = p.find(".dd-select").css("height");
        var n = p.find(".dd-selected-description");
        var m = p.find(".dd-selected-image");
        if (n.length <= 0 && m.length > 0) {
            p.find(".dd-selected-text").css("lineHeight", o)
        }
    }

    function i(m) {
        m.find(".dd-option").each(function() {
            var q = f(this);
            var o = q.css("height");
            var p = q.find(".dd-option-description");
            var n = m.find(".dd-option-image");
            if (p.length <= 0 && n.length > 0) {
                q.find(".dd-option-text").css("lineHeight", o)
            }
        })
    }
})(jQuery);;